...
<form>
	...
	<table width="800"  class="table table-hover">
		...
		<?php 
		$sel = "select * from tbl_complaint c inner join tbl_user u on c.user_id=u.user_id where to_status ='1'and cmp_status = '0'";
		$res = $con->query($sel);
		if($res->num_rows!=NULL)
		{
			...
			while($row = $res->fetch_assoc())
			{
				...
				<tr>
					...
					<td class="btn btn-outline-primary m-2"><a href="Sent_replay.php?rpid=<?php echo $row["cmp_id"]; ?>">Replay</a></td>
				</tr>
				<?php
			}
		}
	...
	<table width="800" class="table table-hover">
		...
		<?php 
		$sel1 = "select * from tbl_complaint c inner join tbl_pajayathe p on c.pajayathe_id=p.pajayathe_id where to_status ='1'and cmp_status = '0'";
		//echo $sel1;
		$res1 = $con->query($sel1);
		if($res1->num_rows!=NULL)
		{
			...
			while($row1 = $res1->fetch_assoc())
			{
				...
				<tr>
					...
					<td class="btn btn-outline-primary m-2"><a href="Sent_replay.php?rpid=<?php echo $row1["cmp_id"]; ?>">Replay</a></td>
				</tr>
				<?php
			}
		}
